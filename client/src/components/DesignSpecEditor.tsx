...
export const DesignSpecEditor = forwardRef<DesignSpecEditorRef, DesignSpecEditorProps>((props, ref) => {
  ...
  const { data: reqSpecDocs = [] } = useDocuments({ reqId, type: 'requirement_spec' });

  const latestReqSpecId = reqSpecDocs[0]?.id;

  const body = useMemo(() => ({ reqId, reqDocId: latestReqSpecId }), [reqId, latestReqSpecId]);



  const { generating, stream, genLog, generate, cancel } = useSpecGenerate({

    endpoint: '/api/specs/design/generate',
